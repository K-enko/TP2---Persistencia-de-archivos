...
int main()
{
    ...
    do
    {
        ...
        ifstream imp_file; //cargamos los datos del archivo...
        imp_file.open("FileData.dat", ios::in);
        if(imp_file.is_open())
        {
            while(!imp_file.eof())
            {
                imp_file >> files[numberOfFiles].size;

                imp_file.get( );

                getline(imp_file, files[numberOfFiles].name);
